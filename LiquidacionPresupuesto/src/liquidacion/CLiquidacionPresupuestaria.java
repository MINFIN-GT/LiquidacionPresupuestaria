...
public class CLiquidacionPresupuestaria {
	...
	private static void agregarIscvGasto(CDatabase dbpago_gastos, ResultSet rsISCV, ResultSet rsGastos, Double pago,
		...
		dbpago_gastos.addBatch(rsISCV.getDouble("formulario"), rsISCV.getDouble("documento"), rsISCV.getDouble("cod_municipio"),
				rsGastos.getDouble("no_cur"), rsGastos.getDouble("entidad"), rsGastos.getDouble("unidad_ejecutora"),
				rsGastos.getDouble("programa"), rsGastos.getDouble("subprograma"), rsGastos.getDouble("proyecto"),
				rsGastos.getDouble("actividad"), rsGastos.getDouble("obra"), rsGastos.getDouble("renglon"),
				rsGastos.getDouble("geografico"), pago < 0 ? pago * -1 : pago, tipo);
